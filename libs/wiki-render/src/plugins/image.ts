 ...
 * @param token 目标图片 Token
 */
function cleanConflictingAlignment(logger: ConsolaInstance, token: Token) {
 ...
 * @param token 目标图片 Token
 */
function validateScale(logger: ConsolaInstance, token: Token) {
 ...
 */
function logMissingImageInfo(logger: ConsolaInstance, hasTitle: boolean, hasSource: boolean, imageSource = 'unknown') {
...
const image: PluginWithOptions<imagePluginOptions> = (md, options) => {
	...
	md.core.ruler.after('inline', 'wiki_image_clean_token', (state) => {
		...
		for (let index = 0; index < tokens.length; index++) {
			...
			if (!hasTitle || !hasSource) {
				logMissingImageInfo(logger, hasTitle, hasSource, rawUrl || 'unknown');
			...
			if (nextToken?.type === 'text') {
				...
				if (match) {
					...
					cleanConflictingAlignment(logger, imgToken); // 清理段落
					validateScale(logger, imgToken); // 校验 scale 属性
	...
	md.renderer.rules['wiki_custom_image'] = (tokens, index) => {
		...
		return `<WIkiMarkdownImage ${properties.join(' ')} />`;
	};
...
export { image };
export type { ImageEnvironment, imagePluginOptions };
