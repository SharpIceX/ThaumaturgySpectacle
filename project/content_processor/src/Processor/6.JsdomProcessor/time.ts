...
const main: subProcessorFunction = (document, data): string => {
	...
	createdElement.textContent = `创建于 ${time?.created.toFormat('yyyy年M月d日 H时m分s秒')}（UTC+8）`;

	const updatedElement = document.createElement('p');
	updatedElement.className = 'updated-time';
	updatedElement.textContent = `更新于 ${time?.updated.toFormat('yyyy年M月d日 H时m分s秒')}（UTC+8）`;
